...
function Get-OpenTcpPorts {
    ...
    Write-Progress -Id $ProgressId -ParentId $ParentProgressId -Activity "Port scan" -Status "Checking ${IPAddress}" -PercentComplete 10
    ...
    if ($PSVersionTable.PSVersion.Major -ge 7) {
        ...
        for ($p = 0; $p -lt $PortCount; $p++) {
            ...
            Write-Progress -Id $ProgressId -ParentId $ParentProgressId -Activity "Port scan" -Status "Checking ${IPAddress}:$Port" -PercentComplete $Pct
    ...
    Write-Progress -Id $ProgressId -ParentId $ParentProgressId -Activity "Port scan" -Completed
...
function Get-ActiveIpsFromSubnet {
    ...
    if ($PSVersionTable.PSVersion.Major -lt 7) {
        for ($i = 0; $i -lt $Total; $i++) {
            ...
            Write-Progress -Id 1 -Activity "Scanning subnet" -Status "Pinging $($Targets[$i])" -PercentComplete $Pct
            if (Test-Connection -ComputerName $Targets[$i] -Count 1 -Quiet -ErrorAction SilentlyContinue) {
                $Live += $Targets[$i]
            }
        }
        Write-Progress -Id 1 -Activity "Scanning subnet" -Completed
        return $Live
    }
    ...
    for ($offset = 0; $offset -lt $Total; $offset += $EffectiveBatch) {
        ...
        Write-Progress -Id 1 -Activity "Scanning subnet" -Status "Processed $Processed of $Total" -PercentComplete $Pct
    }
...
for ($index = 0; $index -lt $TotalActive; $index++) {
    ...
    Write-Progress -Id 2 -Activity "Mapping active devices" -Status "Mapping $IP ($($index + 1) of $TotalActive)" -PercentComplete $Percent
...
Write-Progress -Id 2 -Activity "Mapping active devices" -Completed
...
Write-Progress -Id 3 -Activity "Saving output" -Status "Writing JSON file" -PercentComplete 50
$NetworkTree | ConvertTo-Json -Depth 8 | Out-File $OutputFile -Encoding utf8
Write-Progress -Id 3 -Activity "Saving output" -Completed
...
try {
    ...
    Write-Progress -Id 4 -Activity "Uploading result" -Status "Sending map to API" -PercentComplete 50
    $RequestBody = @{
        name = $MapName
        map = $NetworkTree
    ...
    Write-Progress -Id 4 -Activity "Uploading result" -Completed
